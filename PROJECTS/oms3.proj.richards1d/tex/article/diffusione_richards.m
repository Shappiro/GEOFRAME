...
if Lambda<=0||Lambda>0.5 
    disp('Il parametro Lamba deve essere compreso tra 0 e 0.5 affinchè
     la soluzione sia stabile.')
    disp('Correggi gli intervalli spaziali e temporali per ovviare al 
        problema.');
...
for j=2:(J)     
  for i=2:(I-1)
      Theta(i,j)=
      Lambda*Theta((i-1),(j-1))+
      (1-2*Lambda)*Theta(i,(j-1))+
      Lambda*Theta((i+1),(j-1));
  end   
end
...
% Etichette
title('Contenuto d''acqua VS distanza dalla cima della 
    colonna di suolo')
...
for j=1:T_int:J    
    ...
    if round((j-1)/(3*T_int))== (j-1)/(3*T_int) 
        ...
        etichetta_tempo = strcat('\leftarrow T = ',
            num2str(round(j*Delta_t/100)*100),' secs');  
        text(L/4,Theta(I/4,j),etichetta_tempo,'HorizontalAlignment','left')
    end
end
...
%-----------------------------------------------------------
inf_rate=D*(Theta(1,:)-Theta(2,:))/(Delta_x);
 
figure
hold
 
title('Tasso di infiltrazione (m/s) VS tempo (s) sulla cima 
    della colonna di suolo')
...
title('Infiltrazione cumulata (m) VS tempo (s) sulla cima della
    colonna di suolo')
